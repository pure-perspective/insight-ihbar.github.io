(() => {
  ...
  function contentGroups() {
    ...
    semanticBlocks().forEach((node) => {
      if (node.matches("template[data-slide-only]")) {
        flush();
        groups.push({
          title: node.dataset.slideTitle || "Slide-only content",
      ...
      if (current.weight + weight > pageWeightLimit && current.nodes.length > 1) {
        const previousTitle = current.title.replace(/ \(cont\. \d+\)$/, "");
        flush();
        continuation += 1;
        current = {
          title: `${previousTitle} (cont. ${continuation})`,
          anchor: node,
          nodes: [],
          weight: 0,
          divider: false
  ...
  function slideOnlyContent(template) {
    ...
    if (selector) {
      ...
      if (referenced) {
        ...
        warning.textContent = `Slide reference not found: ${selector}`;
  ...
  function slideShell(title, options = {}) {
    ...
    foot.innerHTML = `<span>Live-synced from source HTML</span><span class="qhs-number"></span>`;
  ...
  function goTo(nextIndex, options = {}) {
    ...
    liveRegion.textContent = `Slide ${currentIndex + 1} of ${slides.length}. ${labelFor(slides[currentIndex], currentIndex)}`;
    updateThumbState();
